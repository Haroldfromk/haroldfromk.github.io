...
돌아보면 이건 **같은 데이터를 저장하는 길이 두 개**인데 한쪽만 단위를 맞춰둔 문제였다. 아이폰이 직접 뛴 기록은 `PFDView`를 거치면서 km로 바뀌고, 워치에서 넘어온 기록은 그 길을 안 지나가니 미터 그대로 들어간다. 322km라는 숫자가 눈에 확 띄어서 금방 찾았지만, 만약 단위가 아니라 소수점 자리 정도만 어긋났으면 한참 못 봤을 것 같다.
...
그리고 이 코드를 다시 보다가 걸리는 게 하나 더 있다. 두 번째 줄의 `guard session.isReachable else { return }`이다.

`transferUserInfo`를 쓴 이유가 [이전글](https://haroldfromk.github.io/posts/RunningProject-(12)/){:target="_blank"}에서 정리했듯 **상대 앱이 꺼져 있어도 큐에 쌓아뒀다가 나중에 전달해주기 때문**인데, 앞에 `isReachable` 가드를 세워두면 상대가 안 닿는 순간엔 아예 큐에 넣지도 못하고 그냥 반환된다. 굳이 큐 방식을 고른 장점을 스스로 막아버린 셈이다. `sendMessage`용 가드를 습관적으로 붙인 것으로 보인다.
