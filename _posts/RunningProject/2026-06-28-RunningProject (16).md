...
건강 앱이 들고 있는 "걷거나 뛴 거리"는 하루 종일 주머니에 넣고 다닌 걸음까지 다 포함한다. 그러니까 캘린더에 3km라고 찍혀 있어도 그게 러닝인지 출퇴근인지 구분이 안 된다. **RunWay가 보여줘야 하는 건 "이 앱으로 기록한 러닝"**이라, 우리가 직접 저장한 데이터를 쓰는 게 맞다.
...
구분 기준은 하나로 잡았다. **이걸 알려주면 사용자가 뭔가 할 수 있는가.** 위치 권한이 거부된 상황은 사용자가 설정에서 켤 수 있으니 알려야 하고, 다이나믹 아일랜드가 안 뜨는 건 러닝 자체엔 아무 영향이 없으니 굳이 방해할 이유가 없다. 아무거나 다 띄우면 사용자는 결국 아무것도 안 읽게 된다.
