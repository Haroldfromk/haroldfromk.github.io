...
확실히 **Swift 5**를 쓰다가 최근에 **Swift 6**을 접하니 `Concurrency`(동시성) 부분이 상당히 빡셌다.

여기서 말하는 동시성은 앱이 여러 일을 한꺼번에 처리하는 걸 말한다. 러닝 앱이라면 GPS 위치를 계속 받으면서, 심박도 받으면서, 화면도 1초마다 갱신해야 한다. 이 여러 갈래가 같은 값을 동시에 건드리면 값이 깨지거나 앱이 예고 없이 죽는데, Swift 6는 그럴 소지가 있는 코드를 **실제로 돌려보기 전에 에러로 막아준다.**

그래서 처음엔 멀쩡해 보이는 코드가 빌드조차 안 되는 일이 잦았다. 그래도 나중에 원인 모를 크래시를 쫓아다니는 것보단 낫고, 무엇보다 나중에 뜯어고치는 것보다 처음부터 6으로 시작하는 게 훨씬 나을 것이라고 생각했다.
...
세 모델을 한 장으로 정리하면 이렇다.

![Day 1에 설계한 Flight, ModeA, Aircraft 세 모델과 그 관계](/assets/img/runway/day1-models.svg){: width="720" height="330"}

첫날이라 코드는 거의 없지만, **이 세 덩어리가 앞으로 나올 거의 모든 이야기의 뼈대가 된다.** `Flight`는 나중에 SwiftData 모델이 되면서 경로 좌표와 경고 이력을 달게 되고, `ModeA`의 `targetPace`와 `paceDeviation`은 GPWS 경고가 언제 뜰지를 정하는 값이 된다.
