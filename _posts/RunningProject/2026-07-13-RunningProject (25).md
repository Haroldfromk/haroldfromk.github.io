...
세 문제 다 원인 파악부터 코드 수정까지 끝내고 실기기로 재테스트를 하다가, 네 번째 문제를 하나 더 발견했다.
...
여섯, 일곱 문제까지 전부 원인 파악부터 코드 수정까지 끝났다. 다시 실기기로 미러링 테스트를 해보고 이상 없으면 App Store Connect에 재제출할 예정이다.
