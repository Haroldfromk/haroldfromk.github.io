...
이 시점에서 지금까지 고친 미러링 버그들을 다시 세어봤다. 원격 종료 이벤트 중복 발행, 앱 종료 시 지도 미표시, 이번 GPS orphan 추적까지 - 절반 이상이 "워치가 주도하고 앱이 실시간으로 따라간다"는 요구사항 자체에서 나오고 있었다. 그래서 미러링 범위를 좁혔다. 앱이 주도할 때만 워치 데이터를 미러링하고, 워치가 주도할 땐 아예 미러링을 시도하지 않고 완전히 독립적으로 뛰게 했다. 종료 후 기록만 iPhone Logbook으로 넘어오면 되는 구조다.
