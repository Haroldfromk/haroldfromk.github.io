...
last_modified_at: 2026-07-15 21:30:00 +0900
...
`elapsedTime`을 어디서 관리할지 고민하다 ViewModel Timer로 정했다. 이 결정이 나중에 Timer 재시작 버그의 씨앗이 됐다. `connect()`가 1회성이라는 걸 몰랐던 게 원인이었고, 정지 후 재시작하면 타이머가 죽는 문제를 겪고 나서야 `autoconnect()` + `sink` 패턴으로 바꿨다.
...
**Actor/AsyncStream 관련 실수는 전부 Week 2에서 처음 나왔다.** continuation 관리, Timer 재시작, 상태 초기화 같은 문제였다. 이후 Week 4의 미러링 재설계에서 다시 마주친 문제들도 사실 이때 이미 한 번씩 겪은 패턴의 변주였다.
...
이 글을 쓸 때만 해도 "남은 건 App Store 심사 제출뿐"이라고 적었는데, 실제로는 거기서 끝나지 않았다.

---

### 제출을 스스로 취소하다

제출 후 대기 중에도 실기기 미러링 테스트를 계속 돌렸다. 워치로 러닝을 시작했는데 다이나믹 아일랜드가 READY 상태에서 움직이질 않았다. 원인을 파보니 iPhone이 자기 GPS 스트림을 받을 때만 화면을 갱신하는 함수가 있었는데, 워치가 주도할 땐 이 함수가 아예 호출되지 않는 경로였다.

더 심각한 건 따로 있었다. 워치 화면을 크라운으로 이탈해도 GPS 추적이 안 멈추는 문제였다. `WatchPFDView`에 정리 로직이 있어야 할 자리에 주석만 있고 실제 코드는 빠져 있었다. 그 결과 이전 러닝의 GPS 추적이 백그라운드에 orphan 상태로 남은 채 새 러닝이 시작되면서, 위치 데이터가 두 소스에서 동시에 흘러들어오는 상황이 벌어졌다. 실제 경로 데이터가 오염될 수 있는 문제라 제출을 그대로 진행할 수 없었다.

이 시점에서 지금까지 고친 미러링 버그들을 다시 세어봤다. 원격 종료 이벤트 중복 발행, 앱 종료 시 지도 미표시, 이번 GPS orphan 추적까지 있었다. 절반 이상이 "워치가 주도하고 앱이 실시간으로 따라간다"는 요구사항 자체에서 나오고 있었다. 그래서 미러링 범위를 좁혔다. 앱이 주도할 때만 워치 데이터를 미러링하고, 워치가 주도할 땐 아예 미러링을 시도하지 않고 완전히 독립적으로 뛰게 했다. 종료 후 기록만 iPhone Logbook으로 넘어오면 되는 구조다.

---

### 리젝을 두 번 더 받다

다시 제출한 뒤 이번엔 통과할 줄 알았는데, 리젝이 두 번 더 왔다.

첫 번째는 Guideline 2.1 Information Needed였다. RunWay는 로그인이 없는 앱이라 심사관이 앱을 어떻게 테스트해야 하는지 알 수 있는 정보가 부족했다. App Review Information의 Notes 필드에 GPS 기반 앱이라 실기기 테스트가 필요하다는 점, 별도 로그인 절차가 없다는 점을 적어서 대응했다.

두 번째는 Guideline 5.2.5, WeatherKit 저작권 표시 누락이었다. WeatherKit을 쓰면 Apple Weather 마크와 legal 페이지 링크를 화면에 표시해야 하는데, 이 부분을 빠뜨리고 있었다. `WeatherService().attribution`으로 마크 이미지 URL과 링크를 받아와 TakeoffView에 붙였다. RunWay는 배경이 항상 어둡게 고정된 테마라 시스템 다크모드 여부와 무관하게 어두운 배경용 마크를 쓰도록 처리했다.

두 리젝 다 버그는 아니었지만, 대응 과정에서 진짜 버그를 두 개 더 잡았다. 하나는 러닝을 멈출 때마다 "Workout Session Error"가 뜨는 문제였는데, 이미 종료된 세션에 `.end()`를 한 번 더 호출하고 있었던 게 원인이었다. 다른 하나는 50미터 이하로 너무 짧게 뛰어서 기록이 저장되지 않았을 때, Summary 화면에 방금 뛴 기록 대신 예전에 저장된 기록이 잘못 표시되던 문제였다.
