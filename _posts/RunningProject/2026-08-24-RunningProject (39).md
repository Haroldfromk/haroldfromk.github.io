...
이 흐름을 한 걸음씩 따라가볼 수 있게 만들었다. 저장 통로와 아이폰 상태를 바꿔가며 네 가지 경우를 다 볼 수 있다.

<iframe
  src="/assets/demo/watch_data_lifecycle_simulator.html"
  width="100%"
  height="920px"
...
네 경우를 다 돌려보면 왜 이 버그가 **가끔만** 났는지가 보인다. 백그라운드 정지 상태에서는 수정 전 코드로도 기록이 사라지지 않는다. 프로세스가 살아 있어서 메모리에 담아둔 게 그대로 남아 있고, 나중에 앱을 열면 그때 저장되기 때문이다. 완전히 종료된 상태에서만 담아둔 것과 함께 통째로 없어진다.

그리고 이게 재현하기 어려웠던 이유이기도 하다. 개발하면서 테스트할 때는 거의 항상 앱을 방금 열어봤던 상태라, 정확히 문제가 되는 조건에 들어갈 일이 별로 없었다.
