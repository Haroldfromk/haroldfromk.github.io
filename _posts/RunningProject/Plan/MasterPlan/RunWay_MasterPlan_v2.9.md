# [Project] RunWay: The Aviator's Running Tracker (Master Plan v2.9)
...
- **ViewModel** — View에 필요한 값만 노출, `elapsedTime` 타이머 관리
...
> ⚠️ 현재 구현은 `preflight`, `cruise`, `touchdown` 3단계. `takeoff`, `approach` 복원 예정 (Day 8).
...
- FlightData 모델 확장 (pace, altitude, heading) ✅
- PFD 실시간 데이터 표시 ✅
- elapsedTime ViewModel 타이머 관리 ✅
- ModeAView 설정 연결 (targetPace, paceDeviation, targetDistance)
- GPWS 로직 실제 연동
