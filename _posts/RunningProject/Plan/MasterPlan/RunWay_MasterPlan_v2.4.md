# [Project] RunWay: The Aviator's Running Tracker (Master Plan v2.4)
...
| 화면 | 설명 | 연결 |
| :--- | :--- | :--- |
| `HomeView` | Flight Deck — Mode 선택, 최근 기록, 주간 차트 | Mission → ModeAView / Free → TakeoffView |
| `ModeAView` | 목표 페이스 + 허용 오차 + 거리 설정 | → TakeoffView |
| `TakeoffView` | Pre-flight Check + ROTATE 카운트다운 | → PFDView |
| `PFDView` | 실시간 계기판 (속도 테이프 + ADI + N1% 게이지) | → TouchdownView |
| `GPWSView` | GPWS 경고 전용 화면 (PFD 오버레이로 대체 가능) | - |
...
struct Flight {
    ...
    let date: Date
    // GPS 좌표 배열 → SwiftData 저장 → MapPolyline으로 경로 표시
...
4. 하단 콘텐츠 즉시 표시 (애니메이션 기다리지 않음)
- 활주로 라이트: 앰버색, 아래쪽 크고 위쪽 작아지는 원근감
...
- `LocationService`, `HealthKitService` 등을 프로토콜로 추상화
- `ServiceContainer`로 관리

### ⚡ Swift Concurrency
- `Task`, `AsyncStream`으로 센서 데이터 흐름 제어
...
- CoreLocation 서비스 (GPS 좌표 수집)
- HealthKit 서비스 (심박수, 케이던스)
- DI 구조 + AsyncStream 연결

### [Week 2] Cockpit & Take-off
- UI 목업 기반 SwiftUI 화면 구현 ✅ (하드코딩)
- PFD 계기판 + CoreMotion Attitude Indicator
- Rotate 이륙 시퀀스 + 햅틱
- GPWS 오버레이 애니메이션
...
- **Phase 3**: AI 기반 맞춤형 페이스 가이드 (iPhone 15 Pro 이상)
