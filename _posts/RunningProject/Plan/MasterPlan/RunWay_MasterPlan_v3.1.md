# [Project] RunWay: The Aviator's Running Tracker (Master Plan v3.0)
...
### FlightData — 러닝 중 실시간 스냅샷
```swift
struct FlightData {
    var distance: Double = 0        // m (RunningCentor에서 누적 계산)
    var phase: FlightPhase = .preflight
    var pace: Double = 0            // min/km (CLLocation.speed → 1 / (speed * 60 / 1000))
    var altitude: Double = 0        // m (CLLocation.altitude)
    var heading: Double = 0         // 0~360° (CLLocation.course)
    var gpwsStatus: GPWSState? = nil // nil = Free Flight (GPWS 비활성)
}
```
- 러닝 중 Actor → AsyncStream → ViewModel → View로 실시간 전달
- `elapsedTime`은 포함하지 않음 — ViewModel 타이머로 별도 관리
...
struct Flight {
    ...
    let time: Int           // seconds (ViewModel 타이머 → 종료 시 저장)
...
struct ModeA {
    ...
    var targetDistance: Double  // km (추가됨)
}
```

### Aircraft — 유저 프로필
```swift
// 온보딩 시 구현 예정 (Week 3로 이동)
// 기종 선택 (A320neo 등)
...
// 저장 정책 고민 중 — 상태 변경 시 1회 저장 or 러닝 단위로 묶어서 표시
```
...
- **ViewModel** — View에 필요한 값만 노출, `elapsedTime` 타이머 관리, `isRunning` 상태 관리
...
actor RunningCentor {
    ...
    private(set) var isReachedPace: Bool = false  // GPWS 활성화 조건
...
> ⚠️ 현재 구현은 `preflight`, `cruise`, `touchdown` 3단계. `takeoff`, `approach` 복원 예정.
...
| MINIMUMS | 목표 거리 50m 전 자동 트리거 | 노란 배경 점멸 | Medium | 경고음 |

### GPWS 활성화 조건
- `isReachedPace` 플래그로 제어 — 허용 오차 범위 내에 한 번이라도 진입한 이후부터 활성화
- 러닝 시작 직후 무효한 페이스 값으로 즉시 트리거되는 문제 방지
- Free Flight(`ModeB`)에서는 `gpwsStatus = nil` — GPWS 비활성
...
### [Week 2] Cockpit & Take-off (진행 중)
- RunningCenter Actor 구현 ✅
  - 위치 데이터 처리 (거리 누적, 경로 좌표 저장) ✅
  - Combine Publisher로 LocationService 리팩토링 ✅
  - FlightPhase 상태 머신 연동 (3단계 구현, 5단계 복원 예정)
- AsyncStream → ViewModel 연결 ✅
- FlightData 모델 확장 (pace, altitude, heading, gpwsStatus) ✅
- PFD 실시간 데이터 표시 ✅
- elapsedTime ViewModel 타이머 관리 ✅
- ModeAView 설정 연결 ✅
- GPWS 로직 실제 연동 ✅
- TakeoffView → PFDView → TouchdownView 화면 흐름 연결 ✅
- AlertsView SwiftData 자동 저장 연동
- MockUI → 실제 데이터 연결 + UI 보완

### [Week 3] Avionics
- Day 11 — AlertsView SwiftData 저장 + FlightSummaryView GPS 경로 연동
- Day 12 — 온보딩 UI + Aircraft 모델 SwiftData 저장
- Day 13 — Watch 단독 앱 UI 구현
- Day 14 — WatchConnectivity 연동
- Day 15 — 심박 / 케이던스 PFD 연동

### [Week 4] Analysis & Stability
- Day 16 — Dynamic Island 5가지 상태
- Day 17 — 실기기 전체 테스트
- Day 18 — 버그 수정 + UI 보완
...
### [Week 5] Release
- Day 21 — App Store 준비 (스크린샷, 설명, 아이콘)
- Day 22 — App Store 심사 제출
- Day 23 — 포트폴리오 포스팅
