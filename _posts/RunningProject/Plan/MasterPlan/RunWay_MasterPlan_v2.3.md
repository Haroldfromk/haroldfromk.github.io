# [Project] RunWay: The Aviator's Running Tracker (Master Plan v2.3)
...
- **GPWS (Ground Proximity Warning System) Monitoring** — 페이스 기준:
    - **"SINK RATE"**: 목표 페이스보다 허용 오차 초과로 느려질 때
    - **"OVERSPEED"**: 목표 페이스보다 허용 오차 초과로 빨라질 때
    - **"GLIDE PATH"**: 목표 페이스 허용 오차 범위 안으로 복귀 시
- **Decision Height**: 목표 거리 도달 500m 전 "MINIMUMS" 안내
- 심박수는 GPWS 기준이 아닌 PFD N1% 게이지로 모니터링용으로만 표시
...
- **PFD Style UI**: 
    - **Airspeed**: 실시간 페이스
    - **Altitude**: 고도
    - **N1% (Engine RPM)**: **심박수**와 **케이던스(SPM)**를 듀얼 게이지로 시각화 (모니터링용)
...
struct ModeA {
    ...
    var paceThreshold: Int  // seconds (허용 오차)
}
```

### User — 사용자 프로필 (온보딩 시 추가 예정)
```swift
// 온보딩 화면 구현 시 추가
// 키, 몸무게, 나이, 성별 → BMI, 최대 심박수(220 - 나이) 자동 계산
```

### 앱 설정
- 간단한 설정값 (단위, 테마 등) → `UserDefaults` 사용
...
- **Onboarding**: TabView(.page) 스타일 온보딩 화면 (User 모델 연동)

### [Week 3] Avionics (워치 및 경고 시스템)
- **Warning**: 페이스 기준 GPWS 로직(Mode A) 구현
