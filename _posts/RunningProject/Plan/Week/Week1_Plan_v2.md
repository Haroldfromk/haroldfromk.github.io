...
| Day 2 | 모델 설계 | Flight, ModeA 순수 모델 설계 |
...
### 목표
러닝 기록과 Mode A 설정을 담을 순수 Swift 모델 설계 (SwiftData 연동은 추후)

### 체크리스트
- [ ] `RunMode` enum 정의 (`.modeA` / `.modeB`)
- [ ] `Flight` 모델 — 러닝 기록 (거리, 시간, 페이스, 심박수, 케이던스, 칼로리, 모드)
- [ ] `ModeA` 모델 — 목표 페이스, 허용 오차(paceDeviation)
