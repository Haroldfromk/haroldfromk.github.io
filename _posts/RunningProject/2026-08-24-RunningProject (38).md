...
func updateUIView(_ uiView: MKMapView, context: Context) {
    // 생략
    uiView.setRegion(routeData.region, animated: false)   // 상태가 바뀔 때마다 매번
}
```
...
### 숫자를 아무리 바꿔도 해결이 안 되는 이유

지금까지는 "GPS 데이터가 8초 이상 안 들어오면 일시정지"였다. 그런데 `LocationService`의 `distanceFilter`가 5m라, **완전히 멈춰 서 있으면 GPS가 멀쩡해도 새 위치 자체가 안 들어온다.** 그러니까 코드가 보는 화면에서는 이 두 가지가 똑같다.

| 실제 상황 | 코드가 보는 것 | 판단 |
|---|---|---|
| 신호등에서 멈춰 섬 | 데이터 없음 | 8초 뒤 일시정지 |
| 터널을 지나 신호가 끊김 | 데이터 없음 | 8초 뒤 일시정지 |

구분할 재료가 없으니 숫자를 어떻게 조절해도 한쪽은 틀린다. 짧게 잡으면 터널에서 오탐이 나고, 길게 잡으면 진짜 멈췄을 때 느리다. 5초일 때 시작 직후 오탐이 났던 것도, 첫 GPS 신호를 아직 못 받은 상태에서 이미 타이머가 돌고 있었기 때문이었다.

---

### 데이터가 오냐 대신 움직이냐를 보기

그래서 판단 재료 자체를 바꿨다. 위치가 멈춰도 계속 들어오게 만들면, 그 안에 실린 속도값으로 정지를 판단할 수 있다.
...
`distanceFilter`는 위치가 최소 몇 m 이상 움직여야 새로 알려줄지 정하는 값이고, 제한을 없애면 기기가 잡을 수 있는 만큼 계속 넘겨준다.

![](https://pub-1fd8ca6711bd4f3f8b74d88a697b50f9.r2.dev/2026-08-24-RunningProject-38/distance-filter-update-timeline.png)

토글을 바꿔가며 두 상황을 나란히 놓고 보면 차이가 분명해진다. 위는 신호등에서 멈춘 경우, 아래는 터널을 지나느라 신호만 끊긴 경우다. 둘 다 6초까지는 똑같이 달린다.

<iframe
  src="/assets/demo/distance_filter_simulator.html"
  width="100%"
  height="600px"
...
5m 설정에서는 **두 타임라인이 6초 이후로 완전히 같다.** 판정도 똑같이 13초에 나온다. 제한을 없애면 멈춘 쪽에는 0.15라는 속도값을 실은 점이 계속 들어오고 터널 쪽은 여전히 비어 있어서, **한쪽은 2초 만에 정지로 확정되고 다른 쪽은 안전망이 15초까지 기다린다.**

배터리가 더 닳지 않을까 걱정했는데, 따져보니 러닝 페이스(초당 3m 안팎)에서는 5m 필터를 쓰던 때도 이미 1.5~2초에 한 번씩 위치가 들어오고 있었다. 차이가 나는 구간은 완전히 멈춰 서 있을 때뿐이라 영향이 미미했고, 실제로 10km 넘게 뛰어봐도 배터리는 무리 없이 버텼다. 오히려 뛰는 동안 위치가 1초에 한 번꼴로 들어오면서 페이스 표시가 더 부드러워지는 덤이 있었다.

---

### 정지 판단에는 가공 전 값을 쓴다

같은 속도값이라도 어디에 쓰느냐에 따라 필요한 성질이 다르다.

| 쓰는 곳 | 필요한 것 | 쓰는 값 |
|---|---|---|
| 화면 페이스 | 숫자가 안 튀는 것 | 다듬은 값 (`smoothingSpeedSecond`) |
| 정지 판단 | 빨리 반응하는 것 | 가공 전 raw `location.speed` |

다듬은 값은 **이전 값에 새 값을 조금씩만 섞어서** 만든 거라, 실제로 멈춰서 raw 속도가 0이 되어도 한동안 높은 채로 남아 있다. 이걸로 정지를 판단하면 몇 초씩 늦는다.

```swift
// RunningCenter.swift (새로 추가한 정지 판단)
...
기준값 0.5m/s는 시속 1.8km다. 보통 걷는 속도가 시속 4~5km, 마트에서 어슬렁거리는 정도가 2~3km니까 그보다도 느린 제자리걸음 수준이다. 그래서 이 아래는 "느리게 걷는 중"이 아니라 **"가만히 서 있는데 GPS가 흔들리는 오차"**로 본다.

raw 값의 약점은 코드에 걸린 세 조건이 각각 막아준다.

| 약점 | 막는 것 |
|---|---|
| 속도를 못 구하면 음수(`-1`)를 반환 | `max(location.speed, 0)` |
| 신호가 튀어서 값 하나가 엉뚱함 | `2초 연속`이라는 조건 |
| 기기 자신도 그 값을 못 믿음 | `speedAccuracy >= 0` |

GPS가 대략 1초에 한 번 들어오니 2초는 **저속 샘플 2번 연속 확인**인 셈이라, 한 샘플만 튀어서는 조건을 못 채운다. `speedAccuracy`가 음수인 샘플은 저속과 정상 어느 쪽으로도 안 세고 그냥 건너뛴다.

---

### 좌표가 흔들리면 속도도 흔들리지 않나

"멈춰 있어도 GPS 좌표는 미세하게 흔들리는데, 그 흔들림이 속도값에도 옮겨붙는 것 아니냐"는 의문이 들 수 있다. 좌표 두 점의 거리를 시간으로 나눠서 속도를 구했다면 실제로 그랬을 것이다.

근데 [`CLLocation.speed`](https://developer.apple.com/documentation/corelocation/cllocation/speed){:target="_blank"}는 그렇게 만들어지는 값이 아니다. GPS 칩이 위성 신호의 **주파수 변화**를 직접 재서 뽑아낸다. 구급차 사이렌이 다가올 때 높게, 멀어질 때 낮게 들리는 것과 같은 원리다. 위성이 어디서 얼마나 빠르게 움직이는지는 이미 알려져 있으니, 받은 신호의 주파수가 얼마나 어긋났는지만 보면 내가 얼마나 빠르게 움직이는지가 거꾸로 나온다.

좌표와는 **아예 다른 경로로 측정되는 값**이라, 좌표가 흔들린다고 속도가 같이 흔들리지는 않는다. ([원리 정리](https://www.onosokki.co.jp/English/hp_e/products/keisoku/automotive/lc8_principle.htm){:target="_blank"})

물론 도심 건물 사이처럼 신호가 반사되는 곳에서는 이 값도 가끔 튄다. 다만 튀는 방향이 항상 "더 빠르게"쪽이라 저속 카운트가 리셋돼서 정지 감지가 몇 초 늦어지는 정도고, 반대로 뛰고 있는데 0 근처로 잘못 찍혀 멈춘 걸로 오판하는 상황은 구조적으로 일어나기 어렵다.

---

### 남은 뒷정리

`distanceFilter`를 없애면서 생기는 부작용을 하나 막아야 했다. 멈춰 있는 동안에도 위치가 계속 들어오면 GPS 오차로 좌표가 미세하게 흔들리면서 누적 거리가 조금씩 늘어난다. 그래서 정지로 판단된 동안은 거리 누적 자체를 건너뛴다.
...
기존의 "N초간 데이터 없음" 타이머는 없애지 않고, GPS가 통째로 끊기는 경우(터널, 엘리베이터)의 안전망으로 남겨뒀다. 더 이상 일반적인 정지 판단 용도가 아니라서 8초에서 15초로 늘렸다.

```swift
// before (RunViewModel.swift / WatchViewModel.swift)
...
결과적으로 진짜 멈췄을 때는 2초 안에 일시정지가 뜨고, GPS 신호만 잠깐 끊긴 경우는 15초까지 달리는 중으로 유지된다. 처음엔 2초가 아니라 3초로 잡았었는데 지인이 "3초도 생각보다 길게 느껴진다"고 해서 한 번 더 낮췄다. 이 로직은 iPhone/Watch가 공유하는 `RunningCenter`에 있어서 두 플랫폼이 한 번에 고쳐졌다.
...
세 패턴을 나란히 놓고 재생해볼 수 있게 만들었다. 진동을 웹에서 그대로 재현할 수는 없으니 같은 리듬을 소리로 들려준다.

<iframe
  src="/assets/demo/haptic_pattern_simulator.html"
  width="100%"
  height="660px"
  style="border: 1px solid rgba(120, 113, 108, 0.2); border-radius: 16px; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.3);"
  scrolling="no"
...
만들어놓고 보니 왜 헷갈렸는지가 눈으로 보였다. 세 패턴 모두 **전체 길이는 0.4~0.45초로 거의 같다.** 결국 손목이 구분하는 건 길이가 아니라 그 안에 몇 번이 들어가느냐다. 수정 전에는 2회 대 3회라 1.5배 차이밖에 안 났고, 4회로 올리면서 정확히 두 배가 됐다.
