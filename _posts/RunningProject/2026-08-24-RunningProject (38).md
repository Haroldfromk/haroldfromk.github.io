...
토글을 바꿔가며 두 상황을 나란히 놓고 보면 이 차이가 분명해진다. 위는 신호등에서 멈춘 경우, 아래는 터널을 지나느라 신호만 끊긴 경우다. 둘 다 6초까지는 똑같이 달린다.
...
5m 설정에서는 **두 상황의 타임라인이 6초 이후로 완전히 같다.** 둘 다 점이 하나도 없고 판정도 똑같이 13초에 나온다. 코드가 받는 정보 자체가 같으니 구분할 도리가 없다.

제한을 없애면 멈춘 쪽에는 0.15라는 속도값을 실은 점이 계속 들어오고, 터널 쪽은 여전히 비어 있다. **한쪽은 2초 만에 정지로 확정되고 다른 쪽은 안전망이 15초까지 기다린다.** 같은 "데이터 없음"이었던 것이 서로 다른 상황으로 갈라진 것이다.
...
멈춰 있어도 위치가 계속 들어오게 만든 다음, `RunningCenter`에서 raw 속도가 일정 시간 이상 낮게 유지되는지로 정지를 판단한다.

여기서 왜 raw 속도를 쓰는지가 중요하다. 페이스 계산에 쓰는 `smoothingSpeedSecond`는 화면 숫자가 튀지 않도록 **이전 값에 새 값을 조금씩만 섞어서** 만든 값이다. 그것도 두 번 겹쳐서. 부드러운 대신 **변화를 늦게 따라간다.** 실제로 멈춰서 raw 속도가 0이 되어도 이 값은 한동안 높은 채로 남아 있어서, 이걸로 정지를 판단하면 몇 초씩 늦어진다.

정지 판단은 빨라야 하고 화면 표시는 부드러워야 한다. **같은 속도값을 목적에 따라 다르게 가공해서 쓰기로 한 것**이고, 그래서 정지 판단에는 가공 전 raw `location.speed`를 그대로 썼다.
