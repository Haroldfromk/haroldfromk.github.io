...
### 2. Watch 종료 시 iPhone은 정지 신호만 처리
...
두 원인이 사실 같은 모양이다. **시작도 종료도 특정 화면에 붙여뒀는데, 워치 주도일 때는 그 화면을 지나가지 않는다.** 시작은 `TakeoffView`의 카운트다운에 걸려 있고, 종료는 터치다운·서머리 화면을 벗어날 때 걸려 있다. 워치가 주도하면 아이폰은 두 화면 다 안 거치니 양쪽 다 안 돈다.

이 모양은 이 프로젝트에서 계속 반복된다. 어떤 동작을 **화면 생명주기에 매달아 두면, 그 화면을 안 지나가는 경로에서는 조용히 실행이 안 된다.** 에러도 안 나고 그냥 아무 일이 없다.
