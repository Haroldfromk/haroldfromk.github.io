...
화면을 덮는 반투명 레이어는 눈에는 "위에 얹힌 안내"로 보이지만, 터치 입장에서는 **그냥 벽**이다. 보이는 것과 만져지는 것이 따로 논다는 걸 놓치기 쉬운데, 하필 일시정지는 러닝을 그만두고 싶을 때 자주 걸리는 상태라 제일 나쁜 조합이었다.
...
여기서 통로를 나눈 기준이 정리된다. **얼마나 급한지가 값마다 다르다.**

| 보내는 값 | 주기 | 왜 |
|---|---|---|
| `flightData` | 3초에 한 번 | 덩치가 크다. 페이스·거리는 3초 늦어도 티가 안 난다 |
| `elapsedTime` | 1초마다 | 숫자 하나뿐이라 가볍고, 3초마다 튀면 바로 눈에 띈다 |
| `pauseData` | 바뀌는 즉시 | 자주 안 바뀌지만 늦으면 한쪽만 계속 뛰는 것처럼 보인다 |

같은 블루투스 한 줄을 쓰는데도 셋을 다르게 다루는 이유는, **크기와 급한 정도가 서로 반대**이기 때문이다. 제일 무거운 게 제일 안 급하고, 제일 가벼운 게 제일 급하다. 이걸 한 통로에 다 태우면 제일 느린 쪽에 전부 맞춰진다. [11번 글](https://haroldfromk.github.io/posts/RunningProject-(11)/){:target="_blank"}에서 상태를 위치 묶음에 얹었다가 뺐던 것과 같은 이야기다.
