...
`xcodebuild build`로 컴파일 확인 후, 실기기(짧은 700m 구간)에서 OVERSPEED → NORMAL → SINK RATE → NORMAL이 정상적으로 돌아오는 것까지 확인했다. 버전은 1.4 작업과 구분하기 위해 1.3.1로 올렸다.
...
`xcodebuild build`로 컴파일 확인했고, 빌드 번호를 5로 올려서 이전에 확인한 700m 테스트 빌드와 구분해뒀다. 두 수정 다 짧은 구간 확인은 끝났고, 실제로 길게 뛰면서(GPS 신호가 흔들리는 구간 포함) 재현 검증하는 절차가 남아있다. 이 부분은 확인되는 대로 이어서 적을 예정이다.
