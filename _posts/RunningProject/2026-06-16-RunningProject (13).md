...
"큐가 밀린다"는 말이 잘 안 와닿아서 왜 그렇게 되는지를 그려봤다. 실제 측정값은 아니고 구조를 보여주는 모형이다.

<iframe
  src="/assets/demo/bt_throttle_simulator.html"
  width="100%"
  height="660px"
...
핵심은 **들어오는 양이 처리되는 양보다 많으면 그 차이가 계속 쌓인다**는 것이다. 조금만 많아도 시간이 지날수록 줄이 길어지지, 어느 선에서 멈추지 않는다. 1초 주기로 60초를 돌리면 120개를 보내서 71개만 도착하고 42개가 버려지며, 도착한 것도 평균 5.4초 늦는다.

**5.4초 늦게 도착한다는 게 곧 "워치와 아이폰이 서로 다른 숫자를 보여준다"는 뜻이다.** 5번 문제(PFD 표시값 불일치)와 6번 문제(종료 동기화 미작동)가 따로 난 게 아니라 같은 원인에서 나온 거였다.

3초 주기로 바꾸면 초당 0.67건이 되어 처리 용량 안으로 들어온다. 버려지는 것도 없고 지연도 0.4초다. 대신 화면 갱신이 뜸해지는데, 그건 2순위로 적어둔 이동 평균으로 덮는다.
