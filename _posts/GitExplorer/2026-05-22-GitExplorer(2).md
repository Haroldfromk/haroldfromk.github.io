...
`_viewModel`에서 `_`는 Property Wrapper의 래퍼 객체에 직접 접근하는 방식이다. `viewModel`이 `@StateObject`로 선언되어 있으면 실제 값은 `viewModel`, 래퍼 객체는 `_viewModel`로 접근한다. `init`에서 초기화할 때는 래퍼 객체에 직접 접근해야 하기 때문에 `_`가 필요하다.
